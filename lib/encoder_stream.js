...
Encoder.prototype.header = function () {
	...
	var descriptor = output.slice(SIZES.MAGIC)
...
Encoder.prototype.compress_DataBlock = function (data) {
	...
	if (compressedSize > 0) {
		...
		compressed = compressed.slice(0, compressedSize - 1)
		buf = buf.slice(0, SIZES.DATABLOCK_SIZE + compressed.length + dbChecksumSize)
	} else {
